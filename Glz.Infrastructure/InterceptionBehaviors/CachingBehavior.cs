            ...
            switch (cachingAttribute.Method)
            {
                ...
                case CachingMethod.Put:
                        ...
                        input.Arguments.Count > 0)
                    {
                        ...
                        {
                            sb.Append(JsonConvert.SerializeObject(input.Arguments[i]));
                    ...
                    case CachingMethod.Get:
                        try
                        {
                            if (CacheManager.Instance.Exists(key, valKey))
                            {
                                ...
                                // 解决NewtonsoftJson序列化的问题
                                if (obj is JObject || obj is JArray)
                                {
                                    obj = JsonConvert.DeserializeObject(obj.ToString(), ((MethodInfo)input.MethodBase).ReturnType);
                                }
